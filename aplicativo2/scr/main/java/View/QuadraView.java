...
public class QuadraView {
    ...
    public void exibirDescricaoQuadra(ListaQuadras listaQuadras, int posicao) {
        if (posicao >= 0 && posicao < listaQuadras.tamanho()) {
            System.out.println("Descricao:" + listaQuadras.getQuadra(posicao).getDescricao());
        } else {
            System.out.println("Posição inválida.");
        }
    }

    public void exibirDiasDisponiveis(ListaQuadras listaQuadras, int posicao) {
        if (posicao >= 0 && posicao < listaQuadras.tamanho()) {
            System.out.println("Dias e horários disponíveis:");
            listaQuadras.getQuadra(posicao).getDiasHorarios().forEach((data, horarios) -> {
